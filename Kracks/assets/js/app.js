...
var _registrationSteps = _interopRequireDefault(require("./components/registration-steps"));
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  _notification["default"].init();

  _registrationSteps["default"].init();
...
},{"./components/burger-menu":3,"./components/notification":4,"./components/popup":5,"./components/registration-steps":6,"./components/select":7,"./components/slider":8,"./components/tabs":9}],3:[function(require,module,exports){
"use strict";
...
var CLOSE = document.querySelector('.js-burger-close');
...
var burgerMenu = function () {
  ...
  var openBurger = function openBurger() {
    ...
    CLOSE.classList.add(CLASS_ACTIVE);
  };
  ...
  var closeBurger = function closeBurger() {
    ...
    CLOSE.classList.remove(CLASS_ACTIVE);
  };
...
exports["default"] = void 0;
var prompt = document.querySelector(".crips-prompt"); // const CLASS_ACTIVE = 'active';

var promptFunc = function () {
  var hidePrompt = function hidePrompt(activePrompt) {
    activePrompt.classList.add("hidden");
  };

  var hidePromptpInit = function hidePromptpInit() {
    prompt.addEventListener("click", function (event) {
      var btn = event.target.closest(".crips-prompt__btn");
      if (!btn) return;
      hidePrompt(event.currentTarget);
    });
...
var popupsFunc = function () {
  ...
  var hidePopup = function hidePopup(activePopup) {
    ...
    if (activePopup.querySelector(".active[data-content]") && activePopup.querySelector(".active[data-tab]")) {
      activePopup.querySelector(".active[data-content]").classList.remove(CLASS_ACTIVE);
      activePopup.querySelector(".active[data-tab]").classList.remove(CLASS_ACTIVE);
    }
  };
...
exports["default"] = void 0;
var checkSteps = document.querySelectorAll('.js-check');
var progressBar = document.querySelector('.js-progress-bar');
var stepNumber = document.querySelectorAll('.js-step-number');
var stepsWrap = document.querySelector('.js-steps-wrap');
var CLASS_ACTIVE = "active";
var CLASS_HIDDEN = "hidden";

function findActiveStep() {
  var activeStep = stepsWrap.querySelector(".js-check.active");
  var stepIndex = Array.from(stepsWrap.children).findIndex(function (item) {
    return item === activeStep;
  });
  return stepIndex;
}

var registrationSteps = function () {
  var nextStep = function nextStep(currentCheck, nextCheck) {
    nextCheck.classList.add(CLASS_ACTIVE);
    currentCheck.classList.remove(CLASS_ACTIVE);
    currentCheck.classList.add(CLASS_HIDDEN);
    var activeStepIndex = findActiveStep();
    var foundStrip = Array.from(progressBar.children).find(function (_, index) {
    ...
    foundStrip.classList.add(CLASS_ACTIVE);
    stepNumber.forEach(function (item) {
      return item.firstElementChild.textContent = activeStepIndex + 1;
    });
  };

  var prevStep = function prevStep(currentCheck, prevCheck) {
    prevCheck.classList.add(CLASS_ACTIVE);
    prevCheck.classList.remove(CLASS_HIDDEN);
    currentCheck.classList.remove(CLASS_ACTIVE);
    var activeStepIndex = findActiveStep();
    Array.from(progressBar.children).forEach(function (item, index) {
      return index > activeStepIndex && item.classList.remove(CLASS_ACTIVE);
    });
    stepNumber.forEach(function (item) {
      return item.firstElementChild.textContent -= 1;
    });
  };

  var registrationStepsInit = function registrationStepsInit() {
    checkSteps.forEach(function (item) {
      return item.addEventListener('click', function (event) {
        var nextBtn = event.target.closest('.js-next-btn');
        var prevBtn = event.target.closest('.js-prev-btn');
        var nextCheckStep = event.currentTarget.nextElementSibling;
        var prevCheckStep = event.currentTarget.previousElementSibling;
        var currentCheckStep = event.currentTarget;

        if (nextBtn) {
          nextStep(currentCheckStep, nextCheckStep);
        }

        if (prevBtn) {
          prevStep(currentCheckStep, prevCheckStep);
        }
      });
    });
  };
...
var _default = registrationSteps;
exports["default"] = _default;
...
var prizesSwiper = document.querySelector('.prizesSwiper');
...
var swiperSlider = function () {
  ...
  var swiperPrizes = new Swiper('.prizesSwiper', {
    navigation: {
      nextEl: '.swiper-button-next',
      prevEl: '.swiper-button-prev'
    },
